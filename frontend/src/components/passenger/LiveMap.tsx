...
const busIcon = L.divIcon({
  ...
  html: `<div style="background-color: #0d9488; border: 3px solid #ffffff; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 15px rgba(13, 148, 136, 0.8); animation: pulse 2s infinite;">
...
const stopIcon = L.divIcon({
  ...
  html: `<div style="background-color: #6366f1; border: 2px solid #ffffff; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 10px rgba(99, 102, 241, 0.6);">
...
export const LiveMap: React.FC = () => {
  ...
  // Polyline points connecting stops and current bus
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 uppercase">
            <Gauge className="w-3.5 h-3.5 text-teal-400" /> Current Speed
          </span>
          <p className="text-xl font-extrabold text-white font-mono">{simulatedGps.speedKmH} <span className="text-xs font-normal text-slate-400">km/h</span></p>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 uppercase">
            <Navigation className="w-3.5 h-3.5 text-indigo-400" /> Current Location
          </span>
          <p className="text-sm font-bold text-white truncate">{simulatedGps.currentStopName}</p>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 uppercase">
            <MapPin className="w-3.5 h-3.5 text-amber-400" /> Next Stop
          </span>
          <p className="text-sm font-bold text-white truncate">{simulatedGps.nextStopName}</p>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 uppercase">
            <Clock className="w-3.5 h-3.5 text-pink-400" /> Boarding Stop ETA
          </span>
          <p className="text-xl font-extrabold text-teal-300 font-mono">~{simulatedGps.etaMinutes} <span className="text-xs font-normal text-slate-400">mins</span></p>
        </div>
      ...
      <div className="glass-panel rounded-3xl overflow-hidden border border-slate-800 shadow-2xl relative">
        ...
        {/* Google Maps External Navigation Floating Pill */}
        <div className="absolute top-4 right-4 z-20">
          <a
            href={googleMapsUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="px-4 py-2 rounded-xl bg-slate-950/90 text-teal-300 hover:text-white border border-teal-500/40 text-xs font-bold shadow-xl flex items-center gap-2 backdrop-blur-md transition-all"
          >
        ...
        <div className="h-[450px] w-full">
          ...
          >
            ...
            {/* Route Polyline */}
            <Polyline positions={polylineCoords} color="#0d9488" weight={4} dashArray="8, 8" />
          </MapContainer>
        </div>

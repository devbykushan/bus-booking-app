...
export const CustomDatePicker: React.FC<CustomDatePickerProps> = ({
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+6px)] left-0 z-50 w-64 sm:w-72 bg-white backdrop-blur-2xl border border-slate-200/90 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up p-3 space-y-2.5 text-slate-800">
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <div className="text-center font-extrabold text-slate-800 text-xs tracking-tight">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {dayNamesShort.map((day, idx) => (
              <span key={idx} className="text-[9px] font-extrabold uppercase tracking-wider text-slate-400">
                {day.charAt(0)}
              </span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: firstDayOfWeek }).map((_, idx) => (
              <div key={`blank-${idx}`} className="h-7 sm:h-8" />
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-7 sm:h-8 rounded-lg text-[11px] font-extrabold transition-all relative flex items-center justify-center ${
                    disabled
                      ? 'text-slate-300 bg-slate-50 cursor-not-allowed line-through opacity-40'
                      : selected
                        ? isAmber
                          ? 'bg-gradient-to-tr from-amber-500 to-amber-600 text-white font-black shadow-xs scale-105 z-10'
                          : 'bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-black shadow-xs scale-105 z-10'
                        : today
                          ? isAmber
                            ? 'bg-amber-50 text-amber-800 border border-amber-300 font-black hover:bg-amber-100'
                            : 'bg-blue-50 text-blue-800 border border-blue-300 font-black hover:bg-blue-100'
                          : 'text-slate-800 hover:bg-slate-100 hover:scale-105 active:scale-95'
                  }`}
                >
                  {dayNum}
                  {today && !selected && (
          ...
          {/* Footer Note */}
          <div className="pt-1.5 border-t border-slate-100 flex items-center justify-between text-[9px] text-slate-400 font-medium">
